...
from .forms import RegistrationForm,Formm, UpdateCustomerForm, UpdateCustomerFormTwo, LoginForm
...
def UploadFile(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Your Documents has been saved")
            return redirect(reverse('main:profile'))
    else:
        form = Formm()
        context = {
...
class MainView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # featured_cities          = City.objects.filter(slug__in = ["Nottingham" , "other"]).annotate(num_property = Count("property")).order_by("-num_property")
